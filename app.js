...
let recipes=[], selected=new Set(JSON.parse(localStorage.getItem("selectedRecipes")||"[]"));
...
function bind(){
  ...
  $("#dialogSelect").onchange=e=>{
    const id=Number(e.target.dataset.id); toggleSelected(id,e.target.checked); render();
  };
}
function showView(v){
...
function save(){localStorage.setItem("selectedRecipes",JSON.stringify([...selected]));updateSelectedCount();}
...
function renderShopping(){
  ...
  recipes.filter(r=>selected.has(r.id)).forEach(r=>r.shopping.forEach(x=>{
    ...
    const old=totals.get(key)||{name:x.name,qty:0,unit:x.unit};
    const q=Number(x.qty); if(Number.isFinite(q))old.qty+=q; totals.set(key,old);
  }));
